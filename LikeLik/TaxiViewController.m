...
- (void)viewDidLoad
{
    ...
    for (int i = 0; i < numberOfViews; i++) {
        ...
        awesomeView.image = [UIImage imageNamed:[self.Photo objectAtIndex:i]];
        if ([UIImage imageNamed:[[NSString alloc] initWithFormat:@"%@",[self.Photo objectAtIndex:i]]].size.height == 640.0) {
            awesomeView.frame = CGRectMake(xOrigin, self.view.center.y/2, self.view.frame.size.width, [UIImage imageNamed:[[NSString alloc] initWithFormat:@"%@",[self.Photo objectAtIndex:i]]].size.height/4);
        }
